...
export const StudentExamDetail = () => {
  ...
  const { id } = useParams(); // id của studentExam
  ...
  useEffect(() => {
    const fetchExam = async () => {
      if (!id || !user?.id) {
        setError("Invalid exam detail request.");
        return;
      }
      ...
      try {
        ...
        console.log("StudentExam User ID:", data?.userId);

        if (user.id !== data?.userId) {
          setError("You are not authorized to view this exam detail.");
          return;
        }
      } catch (err) {
